describe('Get API Tests for all users and single user', () => {
  ...
  it('should fetch all data and verify structure', function() {
    ...
    cy.request(`http://qachallenge.ro/api/test_api.php?${actionForAllUsers}`)
      .then((response) => {
  ...
  it('Get Single user positive testing', () => {
    ...
    cy.request(`http://qachallenge.ro/api/test_api.php?action=${actionForSingleUser}&id=${ValidId}`)
      .then((response) => {
        let responseBody;
        try {
          responseBody = JSON.parse(response.body);
